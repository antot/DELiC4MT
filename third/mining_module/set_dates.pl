...
foreach my $doc_elem ($tree->getDocumentElement->findnodes("//doc")) {
    ...
    foreach my $location ($tree2->getDocumentElement->findnodes("/KAF2/dates/date")) {
	my $lid = $location->getAttribute("did");
	foreach my $loc_child ($location->childNodes()) {
	    if ($loc_child->nodeName eq "dateInfo") {
		#foreach my $place ($loc_child->childNodes()) {
		$lid_place{$lid} = $loc_child; 
	    }
	...
	if (exists($sent_loc{$term_sent{$target}})) { 
	    my $target_location = ($lid_place{$sent_loc{$term_sent{$target}}})->cloneNode(1);
